// Условие задачи:
// У вас есть массив книг, представленных в виде объектов. Каждая книга содержит информацию о названии, авторе, годе публикации, жанре, количестве страниц, статусе (в наличии или в аренде) и, если книга в аренде, то еще и количество экземпляров. Необходимо создать функции для решения следующих задач:
...
function filterBooksAfterYear(books, year) {
  ...
  for (let i = 0; i < books.length; i++) {
    if (books[i].year > year) {
      booksAfter2000.push(books[i]);
    }
  }
  return booksAfter2000;
}
function filterBooksAfterYear2(books, year) {
...
function calculateAverageAge(books) {
  let middleYear = 0;
  books.map((book) => {
    middleYear += book.year;
  });
  return Math.round(middleYear / books.length);
}
...
// console.log('Книги, выпущенные после 2000 года:', filterBooksAfterYear2(books, 2000));
...
// console.log(
//   "Книги с более чем 300 страницами:",
//   getBooksWithMoreThan300Pages(books)
// );
